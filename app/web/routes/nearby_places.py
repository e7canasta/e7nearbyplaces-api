...
from e7.ave.places.google.v1.api import get_gplaces
from e7.ave.places.google.v1.client import hidrate_google_places
...
@nearby_places_router.get("/api/v1/google_places/craw_by_latlng")
async def GET_google_places__craw_by_latlng(lat: float, lng: float, district: str, region: str, subregion: str = None, radius: int = 1200, limit: int = 10, offset: int = 0, sort: str = "distance", country: str = "AR", order: str = "asc", place_type: str = "all", orgin_place_id: str = None) -> GetNearbyGPlacesResponse:
    places = hidrate_google_places(lat, lng, radius, district, region, subregion, country)
...
async def GET_nearby_gplaces(lat: float, lng: float, district: str, region: str, subregion: str = None, radius: int = 1200, limit: int = 10, offset: int = 0, sort: str = "distance", country: str = "AR", order: str = "asc", place_type: str = "all", orgin_place_id: str = None) -> GetNearbyGPlacesResponse:
    places = get_gplaces(lat, lng, radius, district, region, subregion, country)
    response = GetNearbyGPlacesResponse(places=places)
    return response
